...
async function getSubscriptions(
  ...
  if (!lastCheck || lastCheck.getTime() + SUBSCRIPTION_CACHE_TIME < Date.now()) {
    // Get the subscriptions for this account (if we haven't checked in a while)
    const subscriptionForAccount = await pushSubscriptionsRepository.getAllTelegramSubscriptionsForAccounts([account])
...
async function sendNotificationToTelegram(
  ...
  // Get the subscriptions for this account
  const telegramSubscriptions = await getSubscriptions(account, pushSubscriptionsRepository).catch((error) => {
...
async function subscribeToNotifications() {
  ...
  const { subscriptionId, cancelSubscription } = await pushNotificationsRepository.subscribe(async (notification) => {
    return sendNotificationToTelegram(notification, pushSubscriptionsRepository)
  })

  logger.info(`[telegram] Subscribed to notifications with ID ${subscriptionId}`)

  // Watch for connection close
  let connectionOpen = true
  connection.on('close', () => {
    logger.error(`[telegram] Queue connection closed! Reconnecting in ${WAIT_TIME / 1000}s`)
...
async function sendMessageTelegram(chatId: string, notification: PushNotification) {
