...
function getExecutionInfoFactory(dataSource: DataSource, apiBaseUrl: string) {
  return async function getExecutionInfo(orderId: string, partIds: string[]): Promise<ExecutionInfo> {
    ...
    try {
      ...
      return orders.reduce(
        (accumulator, order) => {
          return {
            executedBuyAmount: accumulator.executedBuyAmount + BigInt(order.executedBuyAmount),
            executedSellAmount: accumulator.executedSellAmount + BigInt(order.executedSellAmount),
          }
        },
        {
...
function orderbookFactory(dataSource: DataSource, apiBaseUrl: string): Orderbook {
  return {
...
export default fp(async function (fastify: FastifyInstance) {
  fastify.ready(() => {
    ...
    // );
    const mainnet = orderbookFactory(fastify.orm['mainnet'], getApiBaseUrl(SupportedChainId.MAINNET))
