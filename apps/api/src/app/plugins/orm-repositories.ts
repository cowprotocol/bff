...
export default fp(async function (fastify: FastifyInstance) {
  if (!isDbEnabled) {
    fastify.log.warn('Database is disabled. ORM for repositories will not be used.')
    return
  }

  const isProduction = process.env.NODE_ENV === 'production'

  const migrationsDir = resolve(__dirname, '../../../../../libs/repositories/src/migrations')
...
async function printMigrations(dirPath: string) {
  try {
    const files = (await readdir(dirPath)).filter((file) => file.endsWith('.js'))
