...
import { errorSchema, paramsSchema, responseSchema } from './traderStats.schemas'
...
const traderStats: FastifyPluginAsync = async (fastify): Promise<void> => {
  if (!isDuneEnabled) {
    fastify.log.warn('DUNE_API_KEY is not set. Skipping affiliate trader stats endpoint.')
    return
  }
  ...
  fastify.get<{ Params: ParamsSchema; Reply: ResponseSchema }>(
    ...
    async function (request, reply) {
      try {
        ...
        const result = await affiliateStatsService.getTraderStats(request.params.address)
