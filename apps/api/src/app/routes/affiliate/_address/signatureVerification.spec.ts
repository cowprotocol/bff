...
describe('verifyAffiliateSignature', () => {
  it('returns valid for a matching EOA signature', async () => {
    ...
    const signedMessage = await wallet._signTypedData(typedData.domain, typedData.types, typedData.message)
    const client = {
      getBytecode: jest.fn(),
  ...
  it('returns addressIsNotSmartContract when recovery fails for an EOA address', async () => {
    ...
    expect(result).toBe(SignatureCheckResult.addressIsNotSmartContract)
    expect(client.readContract).not.toHaveBeenCalled()
  })
})
