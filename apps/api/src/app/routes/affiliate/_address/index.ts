...
import { AffiliateProgramExportService, affiliateProgramExportServiceSymbol } from '@cowprotocol/services'
...
import { SignatureCheckResult, verifyAffiliateSignature, type AffiliateTypedData } from './signatureVerification'
...
const affiliate: FastifyPluginAsync = async (fastify): Promise<void> => {
  ...
  fastify.get<{
    ...
    async function (request, reply) {
      ...
      try {
        const affiliateEntry = await affiliatesRepository.getAffiliateByWalletAddress({
  ...
  fastify.post<{
    ...
    async function (request, reply) {
      ...
      try {
        ...
        if (res === SignatureCheckResult.invalidAddress || res === SignatureCheckResult.addressIsNotSmartContract) {
      ...
      try {
        const existingByWallet = await affiliatesRepository.getAffiliateByWalletAddress({
          walletAddress,
        })
        ...
        fastify.log.info({ walletAddress, code: affiliateEntry.code }, 'Affiliate code created')
        ...
        if (isDuneEnabled) {
          ...
          void exportService
            ...
            .catch((error) => {
              fastify.log.error({ error }, 'Affiliate program export after create failed')
            })
        }
