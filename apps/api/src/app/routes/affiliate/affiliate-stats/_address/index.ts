...
import { errorSchema, paramsSchema, responseSchema } from './affiliateStats.schemas'
...
const affiliateStats: FastifyPluginAsync = async (fastify): Promise<void> => {
  if (!isDuneEnabled) {
    fastify.log.warn('DUNE_API_KEY is not set. Skipping affiliate stats endpoint.')
    return
  }
  ...
  fastify.get<{ Params: ParamsSchema; Reply: ResponseSchema }>(
    ...
    async function (request, reply) {
      try {
        ...
        const result = await affiliateStatsService.getAffiliateStats(request.params.address)
