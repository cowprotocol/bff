...
export function getViemClients(): Record<SupportedChainId, PublicClient> {
  ...
  viemClients = AllChainIds.reduce<Record<SupportedChainId, PublicClient>>((acc, chainId) => {
    ...
    if (!rpcEndpoint) {
      logger.warn(`RPC_URL_${chainId} is not set. Using default RPC URL for ${chain.name}`)
    }
...
function getDefaultRpcUrl(chain: Chain, rpcEndpoint?: string): Chain['rpcUrls']['default'] {
  if (!rpcEndpoint) {
