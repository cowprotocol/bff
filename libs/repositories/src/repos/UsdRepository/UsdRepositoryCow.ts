...
export class UsdRepositoryCow extends UsdRepositoryNoop {
  ...
  async getUsdPrice(chainIdOrSlug: string, tokenAddress?: string | undefined): Promise<number | null> {
    ...
    const usdcPrice = invertNativeToTokenPrice(new BigNumber(usdcNativePrice), usdDecimals)
    const tokenPrice = invertNativeToTokenPrice(new BigNumber(tokenNativePrice), tokenDecimals)
 ...
 */
function invertNativeToTokenPrice(value: BigNumber, decimals: number): BigNumber {
