...
import { errorResponse, NULL_ADDRESS, okResponse, WETH } from '../../../test/mock'
...
const cowApiClients: Record<SupportedChainId, CowApiClient> = mapSupportedNetworks(mockApi)

const usdRepositoryCow = new UsdRepositoryCow(cowApiClients, mockErc20Repository)
...
describe('UsdRepositoryCow', () => {
  describe('getUsdPrice', () => {
    describe('success cases', () => {
      ...
      it('USD price calculation is correct', async () => {
        ...
        expect(mockApiGet.mock.calls).toEqual([
          ...
          [
            ...
            {
              params: {
                path: { token: USDC[SupportedChainId.MAINNET].address },
              },
            },
          ],
        ])
      ...
      it('Handles receiving SupportedChainId as platform', async () => {
        // Get USD price for WETH
        const price = await usdRepositoryCow.getUsdPrice('ethereum', WETH)
    ...
    describe('error cases', () => {
        ...
        mockApiGet.mockReturnValue(
          errorResponse({
            ...
            error: {
              ...
              description: "This is a new error type we don't, so we expect the repository to throw",
            },
          })
        )
      ...
      it('Handles un-expected errors (I_AM_A_TEA_POT)', async () => {
        ...
        // Get USD price for something is not even an address
        const priceResult = usdRepositoryCow.getUsdPrice(CHAIN_ID, 'this-is-not-a-token')
      ...
      it('Handles receiving an unsupported chainId', async () => {
        let price = await usdRepositoryCow.getUsdPrice('this-is-not-a-chain-id', WETH)
