...
import { getTenderlySimulationLink, TENDERLY_API_BASE_ENDPOINT, TENDERLY_API_KEY } from '../../datasources/tenderlyApi'
import { injectable } from 'inversify'
import { SimulationData, SimulationInput, SimulationRepository } from './SimulationRepository'
...
export class SimulationRepositoryTenderly implements SimulationRepository {
  private logRequest(chainId: SupportedChainId, simulations: TenderlySimulatePayload[]): TenderlyRequestLog {
  ...
  private logResponse(
    ...
    if (this.checkBundleSimulationError(response)) {
      ...
      responseLog.simulationResults = response.simulation_results.map((result) => ({
  ...
  async postBundleSimulation(
    ...
    try {
      ...
      }).then((res) => res.json())) as TenderlyBundleSimulationResponse | SimulationError
      ...
      const balancesDiff = this.buildBalancesDiff(
        response.simulation_results.map((result) => result.transaction?.transaction_info.asset_changes || [])
      )

      const stateDiff = this.buildStateDiff(
        response.simulation_results.map((result) => result.transaction?.transaction_info.call_trace?.state_diff ?? [])
      )
  ...
  buildBalancesDiff(assetChangesList: AssetChange[][]): Record<string, Record<string, string>>[] {
    ...
    return assetChangesList.map((assetChanges) => {
      assetChanges.forEach((change) => {
        ...
        const updateBalance = (address: string, tokenSymbol: string, changeAmount: string) => {
          ...
          const currentBalance = BigNumber.from(cumulativeBalancesDiff[address][tokenSymbol])
      ...
      return structuredClone<typeof cumulativeBalancesDiff>(cumulativeBalancesDiff)
    })
  }
