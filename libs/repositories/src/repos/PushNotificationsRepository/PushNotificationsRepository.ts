...
export const pushNotificationsRepositorySymbol = Symbol.for('PushNotificationsRepository')
...
export interface PushNotificationsRepository {
  ...
  subscribe(callback: (notification: PushNotification) => Promise<boolean>): Promise<QueueSubscription>
  ping(): Promise<boolean>
}

export class PushNotificationsRepositoryRabbit implements PushNotificationsRepository {
  connection: ConnectToChannelResponse | null = null

  messageRetries = new Map<string, number>() // Track message retries by message ID

  constructor(private readonly queueName = NOTIFICATIONS_QUEUE, private readonly maxRetries = MAX_RETRIES) {}
  ...
  async subscribe(callback: (notification: PushNotification) => Promise<boolean>): Promise<QueueSubscription> {
    logger.info(`[PushNotificationsRepository] Waiting for messages in "${this.queueName}" queue`)
    ...
    const consumer = await channel.consume(
      ...
      async (msg) => {
        if (msg !== null) {
          ...
          logger.debug(`[PushNotificationsRepository] Received message ${messageId}`)
          ...
          try {
            ...
            if (consumeMessage) {
              ...
              logger.error(error, `[PushNotificationsRepository] Error processing message. Retrying later`)
              logger.warn(
...
async function connectToChannel(params: ConnectToQueueParams): Promise<ConnectToChannelResponse> {
