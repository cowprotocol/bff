...
describe('Erc20RepositoryCache', () => {
  ...
  beforeEach(() => {
    ...
    erc20RepositoryCache = new Erc20RepositoryCache(mockProxy, mockCache, cacheName, cacheTimeSeconds)
  })
  ...
  it('should fetch from proxy and cache the result if not cached', async () => {
    ...
    expect(mockCache.set).toHaveBeenCalledWith(cacheKey, JSON.stringify(erc20Data), cacheTimeSeconds)
  ...
  it('should cache NULL_VALUE if proxy returns null', async () => {
    ...
    expect(mockCache.set).toHaveBeenCalledWith(cacheKey, 'null', cacheTimeSeconds)
