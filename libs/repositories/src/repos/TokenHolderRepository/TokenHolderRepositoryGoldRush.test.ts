...
describe.skip('TokenHolderRepositoryGoldRush', () => {
  ...
  beforeAll(() => {
    ...
    container.bind<TokenHolderRepositoryGoldRush>(TokenHolderRepositoryGoldRush).to(TokenHolderRepositoryGoldRush)
    tokenHolderRepositoryGoldRush = container.get(TokenHolderRepositoryGoldRush)
    expect(GOLD_RUSH_API_KEY).toBeDefined()
  })

  describe('getTopTokenHolders', () => {
    it('should return the top token holders of WETH', async () => {
      const tokenHolders = await tokenHolderRepositoryGoldRush.getTopTokenHolders(SupportedChainId.MAINNET, WETH)
    ...
    it('should return null for an unknown token', async () => {
      const tokenHolders = await tokenHolderRepositoryGoldRush.getTopTokenHolders(
