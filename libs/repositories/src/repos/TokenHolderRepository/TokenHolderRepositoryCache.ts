...
export class TokenHolderRepositoryCache implements TokenHolderRepository {
  ...
  async getTopTokenHolders(chainId: SupportedChainId, tokenAddress: string): Promise<TokenHolderPoint[] | null> {
    ...
    const key = getCacheKey(...this.baseCacheKey, 'usd-price', chainId, tokenAddress)
    ...
    const tokenHolders = await this.proxy.getTopTokenHolders(chainId, tokenAddress)
  ...
  private async getValueFromCache(props: { key: string }): Promise<TokenHolderPoint[] | null | undefined> {
    const { key } = props
  ...
  private async cacheValue(props: { key: string; value: TokenHolderPoint[] | null }): Promise<void> {
    const { key, value } = props

    const cacheTimeSeconds = value === null ? this.cacheTimeNullSeconds : this.cacheTimeValueSeconds

    await this.cache.set(key, value === null ? NULL_VALUE : JSON.stringify(value), cacheTimeSeconds)
  }
}
