...
describe.skip('TokenHolderRepositoryEthplorer', () => {
  ...
  beforeAll(() => {
    ...
    container.bind<TokenHolderRepositoryEthplorer>(TokenHolderRepositoryEthplorer).to(TokenHolderRepositoryEthplorer)
  ...
  describe('getTopTokenHolders', () => {
    it('should return the top token holders of WETH', async () => {
      const tokenHolders = await tokenHolderRepositoryEthplorer.getTopTokenHolders(SupportedChainId.MAINNET, WETH)
      ...
      expect(Number(tokenHolders?.[0].balance)).toBeGreaterThan(Number(tokenHolders?.[1].balance))
    }, 100000)

    it('should return null for an unknown token', async () => {
      const tokenHolders = await tokenHolderRepositoryEthplorer.getTopTokenHolders(
    ...
    it('should return null for gnosis chain', async () => {
      const tokenHolders = await tokenHolderRepositoryEthplorer.getTopTokenHolders(SupportedChainId.GNOSIS_CHAIN, WETH)

      expect(tokenHolders).toBeNull()
    }, 100000)

    it('should return null for arbitrum one', async () => {
      const tokenHolders = await tokenHolderRepositoryEthplorer.getTopTokenHolders(SupportedChainId.ARBITRUM_ONE, WETH)

      expect(tokenHolders).toBeNull()
    }, 100000)

    it('should return null for polygon', async () => {
      const tokenHolders = await tokenHolderRepositoryEthplorer.getTopTokenHolders(SupportedChainId.POLYGON, WETH)

      expect(tokenHolders).toBeNull()
    }, 100000)

    it('should return null for avalanche', async () => {
      const tokenHolders = await tokenHolderRepositoryEthplorer.getTopTokenHolders(SupportedChainId.AVALANCHE, WETH)
