...
import { AffiliateRecord, AffiliatesRepository, CreateAffiliateInput } from './AffiliatesRepository'
...
export class AffiliatesRepositoryCms implements AffiliatesRepository {
  async getAffiliateByWalletAddress(params: { walletAddress: string }): Promise<AffiliateRecord | null> {
    const { walletAddress } = params
    const normalizedWallet = walletAddress.toLowerCase()
    const response = await cmsGet<StrapiListResponse<AffiliateAttributes>>(AFFILIATE_COLLECTION_PATH, {
  ...
  async getAffiliateByCode(params: { code: string }): Promise<AffiliateRecord | null> {
    const { code } = params
  ...
  async createAffiliate(params: CreateAffiliateInput): Promise<AffiliateRecord> {
    ...
    const response = await cmsPost<StrapiSingleResponse<AffiliateAttributes>>(AFFILIATE_COLLECTION_PATH, {
      data: {
        code,
        walletAddress,
...
async function cmsGet<T>(path: string, query?: Record<string, string | number | boolean>): Promise<T> {
 ...
 */
async function getAffiliatePage({ page = 1, pageSize = PAGE_SIZE }: PaginationParam = {}): Promise<
  StrapiListResponse<AffiliateAttributes>
> {
  return cmsGet<StrapiListResponse<AffiliateAttributes>>(AFFILIATE_COLLECTION_PATH, {
