...
export class UserBalanceRepositoryCache implements UserBalanceRepository {
  ...
  private getCacheKey(chainId: SupportedChainId, userAddress: string, tokenAddress?: string): string {
    const baseKey = `${this.cachePrefix}:${chainId}:${userAddress.toLowerCase()}`
    if (tokenAddress) {
    ...
    try {
      ...
      return cachedBalances.filter((balance) => {
        return tokenAddresses.some((tokenAddress) => {
          return tokenAddress.toLowerCase() === balance.tokenAddress.toLowerCase()
        })
      })
    } catch (error) {
      logger.warn('Error parsing cached balances. Proceeding to fetch fresh data.', error)
      return []
    }
  }
  ...
  async getUserTokenBalances(
    ...
    // const cached = await this.cacheRepository.get(cacheKey);
    const cachedBalances = await this.getCachedBalances(chainId, userAddress, tokenAddresses)

    // Group the cached balances by token address
    const cachedByToken = new Map(cachedBalances.map((balance) => [balance.tokenAddress.toLowerCase(), balance]))
    ...
    const fetchedBalances = await this.userBalanceRepository.getUserTokenBalances(
      chainId,
      userAddress,
