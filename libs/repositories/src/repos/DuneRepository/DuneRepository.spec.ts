...
describe('DuneRepositoryImpl', () => {
  ...
  describe('executeQuery', () => {
    it('should execute a query successfully with parameters', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith('https://api.dune.com/api/v1/query/12345/execute', {
        method: 'POST',
        headers: {
  ...
  describe('getExecutionResults', () => {
    it('should get execution results successfully', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith('https://api.dune.com/api/v1/execution/test-execution-123/results', {
        headers: {
          'X-DUNE-API-KEY': mockApiKey,
        },
      })
  ...
  describe('waitForExecution', () => {
    ...
    it('should validate data with type assertion function', async () => {
      ...
      const isHookData = (
        ...
        return typeof data === 'object' && data !== null && 'environment' in data
      }
    ...
    it('should timeout if execution does not complete', async () => {
      ...
      ).rejects.toThrow('Execution test-execution-123 did not complete within 1000ms')
    })
  })
})
