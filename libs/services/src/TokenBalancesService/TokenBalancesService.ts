...
export interface TokenBalancesService {
  getTokenBalances({ chainId, address }: { chainId: SupportedChainId; address: string }): Promise<TokenBalancesResponse>
...
export class TokenBalancesServiceMain implements TokenBalancesService {
   ...
   */
  private async getTokenInfos(chainId: SupportedChainId, tokenAddresses: string[]): Promise<Erc20[]> {
    const tokens: Erc20[] = []
    ...
    for (const tokenAddress of tokenAddresses) {
      ...
      if (!token) {
        logger.warn(`Token ${tokenAddress} not found for chain ${chainId}. Skipping.`)
        continue
      }
  ...
  async getUserTokenBalances({
    ...
    const balancesPromise = this.userBalanceRepository.getUserTokenBalances(chainId, userAddress, tokenAddresses)
    const tokensPromise = this.getTokenInfos(chainId, tokenAddresses)

    const [balances, tokens] = await Promise.all([balancesPromise, tokensPromise])
    ...
    return balances
      ...
      .filter((balance): balance is UserTokenBalanceWithToken => balance !== null)
  }
}
