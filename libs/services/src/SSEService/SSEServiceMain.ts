...
export class SSEServiceMain implements SSEService {
  ...
  addClient(client: SSEClient): void {
    ...
    logger.info(`SSE client ${client.clientId} connected for user ${client.userAddress} on chain ${client.chainId}`)
  }
  ...
  getClientsForUser(chainId: SupportedChainId, userAddress: string): SSEClient[] {
    ...
    return Array.from(this.clients.values()).filter(
      (client) => client.chainId === chainId && client.userAddress.toLowerCase() === userAddressLowerCase
    )
  }
  ...
  sendToClient(clientId: string, data: string): boolean {
    ...
    if (client) {
      try {
        ...
        logger.error(`Error sending data to SSE client ${client.clientId}:`, error)
  ...
  broadcastToUser(chainId: SupportedChainId, userAddress: string, data: string): void {
  ...
  broadcastInitialBalances(clientId: string, balances: UserTokenBalanceWithToken[]): void {
   ...
   */
  private clientTracksToken(client: SSEClient, tokenAddressLowerCase: string): boolean {
    return client.tokenAddresses.some((address) => address.toLowerCase() === tokenAddressLowerCase)
  }
